...
public class FakturyPanelEdytujDodaj
	extends PanelEdytujDodajObiekt
{
	...
	class ButtonPokazywaniaPaneluListener
		implements ActionListener
	{
		...
		ButtonPokazywaniaPaneluListener( DatePanel panel, JButton b, String textUkryj, String textPokaz )
		{
			this.panel = panel;
			this.button = b;
	...
	/**
	 * @param tytul
	 * @param rodzajFaktury - Faktura.SPRZEDAZ lub ZAKUP
	 * @param editable
	 * @param popupMagazynWFaktury - popup PrawegoKlikMyszy na tabeli Magazyn
	 * @param korekta 
	 * @throws SQLException
	 */
	@SuppressWarnings( "deprecation" )
	public FakturyPanelEdytujDodaj( String tytul, int rodzajFaktury, boolean editable,
			JPopupMenu popupMagazynWFaktury, boolean korekta )
		throws SQLException
	{
		this.rodzajFaktury = rodzajFaktury;
		this.listaProduktow = new ArrayList<ProduktWFakturze>();
		setBorder( new TitledBorder( null, tytul, TitledBorder.CENTER, TitledBorder.TOP, null, null ) );
		...
		{
			////
			String[] kontrachenci = KontrachentBaza.pobierzWszystkieNazwyZBazy();

			comboKontrachenci = new JComboBox<Object>( kontrachenci );
			...
			//comboKontrachenci.setModel( new DefaultComboBoxModel<String>( new String[] { "posortowani kontrahenci" } ) );
		...
		{
			////
			String[] waluta = WalutaManager.pobierzWszystkieZBazy( Waluta.tabelaWaluta, true );

			comboWaluta = new JComboBox<Object>( waluta );
			/*comboWaluta.addActionListener( new ActionListener() {
				@Override
				public void actionPerformed( ActionEvent e )
				{
					zmienKontrachent();
		...
		// data wystawienia faktury
		{
			...
			final JButton dataWystawieniaFakturyButton = new JButton( "Pokaż datę faktury" );
			dataWystawieniaFakturyButton.addActionListener( dataWystawieniaFakturyButtonListener =
				new ButtonPokazywaniaPaneluListener( panelDatyWystawFaktury,
						dataWystawieniaFakturyButton, "Ukryj datę faktury", "Pokaż datę faktury" ) );
			...
			tloPanel.add( dataWystawieniaFakturyButton, gbc_dataWystawieniaFaktury );
		...
		// termin platnosci
		{
			...
			final JButton terminPlatnosciButton = new JButton( "Pokaż termin płatności" );
			terminPlatnosciButton.addActionListener( terminPlatnosciButtonListener =
				new ButtonPokazywaniaPaneluListener( panelTerminPlatnosci, terminPlatnosciButton,
		...
		if ( !korekta ) {
			...
			panelMagazynu = new MagazynPanel( /*wyswietlButtony*/false, popupMagazynWFaktury );
			panelMagazynu.zmienDwuklikFunktor( new FunktorDwuklikAkcjaFaktura( listaWybranychPanel,
					listaProduktow, this ) );
	...
	protected void zmienKontrachent()
	{
		...
		if ( k != null ) {
			...
			for ( ProduktWFakturze _produkt : listaProduktow ) {
				...
				_produkt._cena_jednostkowa = ( ( _produkt.produkt.cena_zakupu ) * mnoznik / 100 );
	...
	public static class FunktorDwuklikAkcjaFaktura
		extends FunktorDwuklikTabela
	{
		...
		public FunktorDwuklikAkcjaFaktura( FakturyPanelListaWybranychAbstrakt listaWybranych,
				List<ProduktWFakturze> listaProduktow, FakturyPanelEdytujDodaj parent )
		{
		...
		public void run( ObiektWiersz wiersz, int rowIndex )
		{
			...
			// usuwanie ilosci
			try {
				parent.magazynZmienIloscProduktow( rowIndex, -ilosc, parent.rodzajFaktury );
			}
			catch ( Exception e ) {
				MojeUtils.showError( e );
				return;
			}
			Produkt produktSlownik = (Produkt) ProduktBaza.pobierzObiektZBazy( id_produktu );
			...
			listaWybranych.tabelaDodajWiersz( new String[] { "" + lp, wiersz.wiersz[1],
				MojeUtils.utworzWartoscZlotowki( cena ), ilosc + "",
				MojeUtils.utworzWartoscZlotowki( ilosc * cena ) } );

			listaProduktow.add( new ProduktWFakturze( lp, cena, ilosc, produktSlownik, false, 0 ) );
		}
	...
	public void uzupelnijFormatke( Object obiekt, JDialog parent )
	{
		...
		if ( listaWybranychPanel instanceof FakturyPanelListaWybranych )
			( (FakturyPanelListaWybranych) listaWybranychPanel ).zaladujListeFaktury();
		else
			( (FakturyKorektaPanelListaWybranych) listaWybranychPanel ).zaladujListeFaktury();
		listaProduktow.addAll( faktura.getProdukty_copy() );
		if ( faktura.isKorekta && faktura.produktyKorekta != null && !faktura.produktyKorekta.isEmpty() ) {
			( (FakturyPanelKorekta) listaWybranychPanel.fakturyPanel ).listaProduktowKorekta.addAll( faktura.getProduktyKorekta_copy() );
		}
		{
			/*Przypadek w którym wczytano fakturę z pliku .xls - nie występuje w niej kontrahent*/
			System.out.println( faktura.kontrahent + " ddsadsadsad" );
	...
	public ObiektZId pobierzZFormatki( int id ) throws Exception
	{
		...
		if ( id == 0 ) {
			...
			return new Faktura( 0, numer, data_wystawienia, termin_platnosci, zaplacona, rodzajFaktury,
					kontrahent, wartosc, true, false, copy_lista, null, waluta );
		}
		else {
			int wartosc = 0;
			for ( ProduktWFakturze p : listaProduktow )
			...
			return new Faktura( faktura.id_faktura, numer, panelDatyWystawFaktury.getDate(),
					panelTerminPlatnosci.getDate(), false, rodzajFaktury, kontrahent, wartosc, true,
					faktura.isKorekta, copy_lista, null, waluta );
		}
	}
	...
	public Kontrachent pobierzKontrachenta()
	{
		if ( comboKontrachenci.getSelectedIndex() < 0 )
			return null;
		// haha ha
		return (Kontrachent) KontrachentBaza.pobierzObiektZBazy( comboKontrachenci.getSelectedItem()
																					.toString() );
	}
	...
	public Long pobierzWalute()
	{
		...
		return WalutaManager.pobierzIdZBazy( comboWaluta.getSelectedItem().toString(),
			Waluta.tabelaWaluta );
	...
	public void usunProduktZListy( int lp )
	{
		for ( ProduktWFakturze _produkt : listaProduktow )
			if ( _produkt.lp == lp ) {
				...
				if ( index >= 0 ) {
					int ilosc = Integer.parseInt( (String) panelMagazynu.getModelValueAt( index, 2 ) );
	...
	/**
	 * @throws w przypadku odjecia zbyt duzej ilosci rzuca bledem
	 */
	public void magazynZmienIloscProduktow( int rowIndex, int wartoscZmiany, int rodzaj ) throws Exception
	{
		int columnIndex = 2;
		int _ilosc_magazyn =
			Integer.parseInt( (String) panelMagazynu.getModelValueAt( rowIndex, columnIndex ) );
		int nowa_wartosc;
		if ( rodzaj == Faktura.SPRZEDAZ ) {
			if ( _ilosc_magazyn + wartoscZmiany < 0 )
				throw new UserShowException( "Nie można odjąć tyle produktów z magazynu!" );
	 ...
	 * @throws w przypadku odjecia zbyt duzej ilosci rzuca bledem
	 */
	public void aktualizujIloscProduktow_panelMagazyn( ProduktWFakturze produkt, int nowa_wartosc,
