...
public class FakturyKorektaPanelListaWybranych
	...
	private final ActionListener korygujListener = new ActionListener() {
		...
		public void actionPerformed( ActionEvent e )
		{
			try {
				...
				String strLP = (String) getModelValueAt( numerUsuwanegoWiersza, 0 );
				if ( strLP.isEmpty() )
					return;
				...
				ProduktWFakturze produktKorekta = fakturyPanel.pobierzProduktZListy( produktLP ).copy();
				...
				int index = fakturyPanel.szukajIndexProduktuWModelu( produktKorekta );
				int ilosc =
					Integer.parseInt( (String) fakturyPanel.panelMagazynu.getModelValueAt( index, 2 ) );
				if ( fakturyPanel.rodzajFaktury == Faktura.SPRZEDAZ )
					ilosc += produktKorekta.ilosc_produktu;
				else
					ilosc -= produktKorekta.ilosc_produktu;
				fakturyPanel.panelMagazynu.setModelValueAt( "" + ilosc, index, 2 );
				//( (FakturyPanelKorekta) fakturyPanel ).panelMagazynu
				...
				( (FakturyPanelKorekta) fakturyPanel ).korektaProduktu( numerUsuwanegoWiersza, produktLP );

				( (FakturyPanelKorekta) fakturyPanel ).listaProduktowKorekta.add( produktKorekta );
			}
			catch ( Exception e1 ) {
	...
	private final ActionListener usunListener = new ActionListener() {
		...
		public void actionPerformed( ActionEvent e )
		{
			try {
				...
				if ( ( (String) getModelValueAt( numerUsuwanegoWiersza, 0 ) ).isEmpty() ) {
					// korekta
					( (FakturyPanelKorekta) fakturyPanel ).usunProduktKorektaZListy( (String) getModelValueAt(
	...
	private final TableModelListener listaWybranychListener = new TableModelListener() {
		...
		public void tableChanged( TableModelEvent e )
		{
			...
			if ( !isKorekta ) {
				...
				if ( wycofaj ) {
					// nie mozemy edytowac produktow ze starej faktury
					// wycofaj
				...
				// edycja korekty
				String data = (String) model.getValueAt( row, column );
				String edytowanyProduktNazwa = (String) model.getValueAt( row, 1 );
				int iloscOryginalnegoProduktu =
					Integer.parseInt( (String) model.getValueAt( row - 1, 3 ) );
				ProduktWFakturze zmienionyProdukt =
					( (FakturyPanelKorekta) fakturyPanel ).pobierzProduktKorekteZListy( edytowanyProduktNazwa );
				...
				if ( column == 2 ) {
					...
					try {
						...
						if ( data.startsWith( "-" ) ) {
							...
							// sprawdz czy nie pobieramy za duzo
							int nowa_wartosc = Integer.parseInt( data.substring( 1 ) );
				...
				zmienionyProdukt.wartoscPoKorekcie =
					zmienionyProdukt._cena_jednostkowa
						* ( zmienionyProdukt.ilosc_produktu + iloscOryginalnegoProduktu );
				model.setValueAt( MojeUtils.utworzWartoscZlotowki( zmienionyProdukt.wartoscPoKorekcie ),
					row, 4 );
			}
		}
	};

	public FakturyKorektaPanelListaWybranych( FakturyPanelEdytujDodaj fakturyPanel )
	{
		try {
			globalny_lp = 0;
