...
public class FakturyPanelKorekta
	extends FakturyPanelEdytujDodaj
{
	 ...
	 * @throws SQLException
	 */
	public FakturyPanelKorekta( String tytul, int rodzajFaktury, boolean editable,
			JPopupMenu popupMagazynWFaktury )
		throws SQLException
	{
		...
		for ( ProduktWFakturze _produkt : listaProduktowKorekta ) {
			...
			for ( int i = 0; i < listaWybranychPanel.getModelRows(); ++i ) {
				if ( listaWybranychPanel.getModelValueAt( i, 0 ) != null
					&& !( (String) listaWybranychPanel.getModelValueAt( i, 0 ) ).isEmpty() )
					continue;
				try {
	...
	public ObiektZId pobierzZFormatki( int id ) throws Exception
	{
		...
		if ( id == 0 ) {
			...
			return new Faktura( 0, numer, data_wystawienia, termin_platnosci, zaplacona, rodzajFaktury,
					kontrahent, wartosc, true, true, copy_lista, copy_lista_korekta, waluta );
		}
		else {
			int wartosc = 0;
			/*for ( ProduktWFakturze p : listaProduktow )
				wartosc += p.liczWartoscZNarzutem();
			for ( ProduktWFakturze p : listaProduktowKorekta )
				wartosc += p.liczWartoscZNarzutem();*/
			out: for ( ProduktWFakturze p : listaProduktow ) {
				for ( ProduktWFakturze pKorekta : listaProduktowKorekta )
					if ( p.produkt.nazwa.equals( pKorekta.produkt.nazwa ) ) {
			...
			return new Faktura( faktura.id_faktura, numer, panelDatyWystawFaktury.getDate(),
					...
					faktura.isKorekta, copy_lista, copy_lista_korekta, waluta );
		}
	}
	...
	{
		listaWybranychPanel.model.insertRow( numerWiersza + 1, new String[] {
			"",//lp
			(String) listaWybranychPanel.model.getValueAt( numerWiersza, 1 ),
			(String) listaWybranychPanel.model.getValueAt( numerWiersza, 2 ),
			"-" + (String) listaWybranychPanel.model.getValueAt( numerWiersza, 3 ),
			//"-" + (String) listaWybranychPanel.model.getValueAt( numerWiersza, 3 ),
			"0" } );


		//listaProduktow.add( new ProduktWFakturze( lp, cena, ilosc, produktSlownik ) );
	}


	public ProduktWFakturze pobierzProduktKorekteZListy( String edytowanyProduktNazwa )
	{
		for ( ProduktWFakturze _produkt : listaProduktowKorekta )
