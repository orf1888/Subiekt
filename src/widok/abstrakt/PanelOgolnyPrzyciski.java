...
public class PanelOgolnyPrzyciski
	extends PanelOgolnyTabela
{
	...
	//ActionListener panelDodajOkButtonListener;
	...
	public void init( PanelOgolnyParametry params ) throws Exception
	{
		...
		sizeY = params.sizeY;
		if ( panelDodaj != null )
			oknoModalneEdytujDodaj =
				new DialogEdytujDodaj( tworzDodajOKListener(), anulujListener, panelDodaj, params.sizeX,
						params.sizeY );
	...
	private void initPrzyciski()
	{
		panel_przycikow = new JPanel();
		{
			...
			gbl_panel_przycikow.columnWidths = new int[] { 0, 0 };
			gbl_panel_przycikow.rowHeights = new int[] { 0, 0, 0, 0 };
			gbl_panel_przycikow.columnWeights = new double[] { 0.0, Double.MIN_VALUE };
			gbl_panel_przycikow.rowWeights = new double[] { 0.0, 0.0, 0.0, Double.MIN_VALUE };
	...
	private void initWyszukiwarka()
	{
		JPanel panel_wyszukiwarki = new JPanel();
		{
			...
			gbl_panel_wyszukiwarki.columnWidths = new int[] { 0, 0, 0 };
			gbl_panel_wyszukiwarki.rowHeights = new int[] { 0, 0 };
			gbl_panel_wyszukiwarki.columnWeights = new double[] { 1.0, 0.0, Double.MIN_VALUE };
			gbl_panel_wyszukiwarki.rowWeights = new double[] { 0.0, Double.MIN_VALUE };
	...
	private ActionListener tworzDodajOKListener()
	{
		return new ActionListener() {
			...
			{
				try {
					//pobierz dane z formatki, id zostanie ustawione w funkcji dodaj(nowy)
	...
	public ActionListener panelEdytujOkButtonListener = new ActionListener() {
		...
		public void actionPerformed( ActionEvent arg0 )
		{
			try {
				...
				ObiektZId nowy = oknoModalne.pobierzZFormatki( staryObiekt.getId() );

				obiektBazaManager.edytuj( staryObiekt, nowy );
				tabelaEdytujWiersz( numerEdytowanegoWiersza, nowy.piszWierszTabeli() );
				ukryjModalneOkno();
			}
			catch ( UserShowException e ) {
				MojeUtils.showError( e );
		...
		return new ActionListener() {
			...
			public void actionPerformed( ActionEvent arg0 )
			{
				try {
					...
					oknoModalne =
						new DialogEdytujDodaj( panelEdytujOkButtonListener, anulujListener, panelEdytuj,
								sizeX, sizeY );
		...
		return new ActionListener() {
			...
			public void actionPerformed( ActionEvent arg0 )
			{
				try {
					...
					if ( MojeUtils.usunWiersz( "Czy na peno chcesz usunąć?", wiersz ) ) {
	...
	protected ActionListener szukajListener = new ActionListener() {
		...
		public void actionPerformed( ActionEvent arg0 )
		{
			...
			try {
				...
				//ObiektWyszukanieWarunki warunki = obiektBazaManager.tworzPoWszystkim( szukanieText, Globals.Widoczny, Globals.SzukajLike );
				String[][] data = obiektBazaManager.pobierzWierszeZBazy( warunki );
				if ( data.length == 0 ) {
					szukanieText += "*";
					warunki.dodajWarunekDowolnaKolumna( szukanieText );
					data = null;		// przeladuje jeszcze raz ponizej
				}
	...
	public static class FunktorDwuklikTabelaAkcja
		...
		public void run( ObiektWiersz wiersz, int row )
		{
			...
			parent.oknoModalne =
				new DialogEdytujDodaj( parent.anulujListener, parent.anulujListener,
						parent.panelWyswietl, parent.sizeX, parent.sizeY );
