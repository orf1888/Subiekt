	...
	extends PanelOgolnyPrzyciski
{
	...
	private JPopupMenu tworzMenuPopup()
	{
		...
		// Guzik usuń
		JMenuItem itemCosTam = new JMenuItem( "Usuń", null /*new ImageIcon( "1.gif" )*/);
	 ...
	 * @param popupMenu - mozna podac wlasny popup, jesli chcemy default to podajemy null
	 */
	public MagazynPanel( boolean wyswietlButtony, JPopupMenu popupMenu )
	{
		try {
			...
			init( new PanelOgolnyParametry( initModelFunktor, /*dodajListener, edytujListener, usunListener, szukajListener,
																anulujSzukanieListener,*/
			Produkt.kolumnyWyswietlane.length - 1, popupMenu, ProduktBaza.instance(), pDodaj, pEdytuj,
					pWyswietl, wyswietlButtony, Produkt.kolumnyWyswietlane ) );
			...
			// filtrowanie po producencie u góry tabelki
			{
				...
				String[] producenci =
					SlownikManager.pobierzWszystkieZBazy( SlownikElement.tabelaProducent, true );
				...
				comboBoxFiltrowanieProducent.addActionListener( new ActionListener() {
					@Override
					public void actionPerformed( ActionEvent arg0 )
					{
						String producent = (String) comboBoxFiltrowanieProducent.getSelectedItem();
						filtruj( producent );
					}
				} );

				listaComboBoxowFiltrProducenta.add( comboBoxFiltrowanieProducent );
			}
		}
		catch ( Exception e ) {
			e.printStackTrace();
	...
	protected void filtruj( String producent )
	{
		try {
			if ( producent.equals( dowolnyProducent ) ) {
				...
				Long producent_id =
					SlownikManager.pobierzIdZBazy( producent, SlownikElement.tabelaProducent );
				if ( producent_id == null )
					return;
				warunki.dodajWarunek( producent_id, "id_producenta" );
			}
			/*warunki.dodajPoProducencie( producent );*/
			//tworzPoWszystkim( szukanieText, Globals.Widoczny, Globals.SzukajLike );
			przeladujTabele();
		}
		catch ( Exception e ) {
			MojeUtils.showPrintError( e );
	...
	private final InitModelFunktor initModelFunktor = new InitModelFunktor() {
		...
		{
			try {
				//ProduktWyszukanieWarunki warunki = (ProduktWyszukanieWarunki) obiektBazaManager.tworzPoWidocznosci( Globals.Widoczny );
