...
public class ProduktPanelEdytujDodaj extends PanelEdytujDodajObiekt
{
	...
	public ProduktPanelEdytujDodaj(String tytul, boolean editable) {
		setBorder(new TitledBorder(null, tytul + " produkt",
		...
		panel = new JPanel();
		panel.setBorder(new TitledBorder(null, "Opcje producenta",
				TitledBorder.CENTER, TitledBorder.TOP, null, null));
	...
	{
		int cenaZakupu = MojeUtils.utworzWartoscGrosze(
				textDomyslnaCenaZakupu.getText(), "Cena zakupu");

		if (cenaZakupu <= 0)
			throw new UserShowException(
					"Podano niepoprawną cenę!\n(Cena musi być dodatnia!)");
		/* walidacja wartości z pól: kod, nazwa */
		if (textKod.getText().replaceAll(" ", "").isEmpty()
				|| textKod.getText().length() < 3)
			throw new UserShowException(
					"Kod powinien składać się z conajmniej 3 znaków!");
		...
		return new Produkt(textKod.getText(), textNazwa.getText(), ilosc, true,
				id_producenta, cenaZakupu, id);
	...
	protected ActionListener dodajListener = new ActionListener()
	{
		public ActionListener okListener = new ActionListener()
		{
			...
			public void actionPerformed(ActionEvent a)
			{
				try
				{
					/* dodaj producenta */
					if (SlownikManager
							.dodaj(ProducentPanelEdytujDodaj.textNazwaProducenta
									.getText()))
					{
						ukryjModalneOkno();
						/*
						 * odśwież widok w panelch magazy i edytujdodajprodukti
						 * ustaw go dodanego producenta jako wyświetlanego
						 */
						comboProducent
								.addItem(ProducentPanelEdytujDodaj.textNazwaProducenta
										.getText());
						comboProducent
								.setSelectedItem(ProducentPanelEdytujDodaj.textNazwaProducenta
										.getText());
						MagazynPanel
								.dodajComboBoxFiltrProducenta(ProducentPanelEdytujDodaj.textNazwaProducenta
										.getText());
					}
				} catch (Exception e)
				{
		...
		public void actionPerformed(ActionEvent arg0)
		{
			...
			oknoModalne = new DialogEdytujDodaj(okListener, anulujListener,
					new ProducentPanelEdytujDodaj(), 550, 425);
	...
	protected ActionListener edytujListener = new ActionListener()
	{
			...
			public void actionPerformed(ActionEvent arg0)
			{
				try
				{
					...
					if (SlownikManager.edytuj(nazwaTmp,
							ProducentPanelEdytujDodaj.textNazwaProducenta
									.getText()))
					{
						ukryjModalneOkno();
						/*
						 * odswież widoki ustaw jako wyświtlany zedytowany
						 * obiekt
						 */
		...
		public void actionPerformed(ActionEvent arg0)
		{
			try
			{
				oknoModalne = new DialogEdytujDodaj(okListener, anulujListener,
						new ProducentPanelEdytujDodaj(), 550, 425);
				ProducentPanelEdytujDodaj
						.uzupelnijProducenta((String) comboProducent
								.getSelectedItem());
				oknoModalne.setDefaultCloseOperation(JDialog.DISPOSE_ON_CLOSE);
				oknoModalne.setSize(450, 124);
				oknoModalne.setVisible(true);
