...
public class KontrachentPanelEdytujDodaj
	extends PanelEdytujDodajObiekt
{
	...
	/**
	 * @param tytul - "Dodaj" lub "Edytuj"
	 * @param editable 
	 */
	public KontrachentPanelEdytujDodaj( String tytul, boolean editable )
	{
		setBorder( new TitledBorder( null, tytul + " kontrahenta", TitledBorder.CENTER,
	...
	public void uzupelnijFormatke( Object obiekt, JDialog parent )
	{
		...
		textKredytKupiecki.setText( MojeUtils.formatujWartosc( kontrachent.kredyt_kupiecki ) );
	}

	/**
	 * pobiera dane z formatki i tworzy z nich nowego kontrachenta.
	 * Musimy zapewnic "id", aby obiekt byl kompletny
	 * @param id - cache'owane gdzies
	 * @return
	 * @throws Exception 
	 */
	@Override
	public ObiektZId pobierzZFormatki( int id ) throws Exception
	{
		...
		//narzut więszkszy od zera
		/*if ( narzut <= 0 )
			throw new UserShowException( "Narzut musi być większy od zera" );
		 */

		/*kredyt kupiecki*/

		int kredytKupiecki =
			MojeUtils.utworzWartoscGrosze( textKredytKupiecki.getText(), "Kredyt kupiecki" );

		if ( kredytKupiecki < 1000 * 100 )
			throw new UserShowException( "Podano niepoprawny kredyt kupiecki (minimum 1000zł)" );

		// --------------- walidacja numerow (za duze by rzutowac na liczbe) ---------------
		/*nip*/
		...
		return new Kontrachent( textNazwa.getText(), textMiasto.getText(), textKodPocztowy.getText(),
				textUlica.getText(), narzut, true, nip, regon, textNrKonta.getText(), kredytKupiecki, id );
	}
