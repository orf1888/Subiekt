...
public class WysylkaPanelEdytujDodaj
	extends PanelEdytujDodajObiekt
{
	 ...
	 */
	@SuppressWarnings( "deprecation" )
	public WysylkaPanelEdytujDodaj( String tytul, boolean editable, JPopupMenu popupMagazynWWysylce )
		throws SQLException
	{
		this.listaProduktow = new ArrayList<ProduktWWysylce>();
		setBorder( new TitledBorder( null, tytul + " wysyłkę", TitledBorder.CENTER, TitledBorder.TOP,
				null, null ) );
		...
		// data 
		{
			...
			dataButton.addActionListener( dataButtonListener =
				new ButtonPokazywaniaPaneluListener( panelData, dataButton, "Ukryj datę", "Pokaż datę" ) );
		...
		{
			panelMagazynu = new MagazynPanel( /*wyswietlButtony*/false, popupMagazynWWysylce );
			panelMagazynu.zmienDwuklikFunktor( new FunktorDwuklikAkcjaWysylka( listaWybranychPanel,
					listaProduktow, this ) );
			splitPane.setLeftComponent( panelMagazynu );
		}
	}

	public static class FunktorDwuklikAkcjaWysylka
		extends FunktorDwuklikTabela
	{
		...
		public FunktorDwuklikAkcjaWysylka( WysylkaPanelListaWybranych listaWybranych,
				List<ProduktWWysylce> listaProduktow, WysylkaPanelEdytujDodaj parent )
		{
		...
		public void run( ObiektWiersz wiersz, int rowIndex )
		{
			...
			listaWybranych.tabelaDodajWiersz( new String[] { "" + lp, wiersz.wiersz[0],
				wiersz.wiersz[1], ilosc + "" } );
	...
	public void uzupelnijFormatke( Object obiekt, JDialog parent )
	{
		...
		//numerField.setText( "" + wysylka.numer );
	}
		...
		if ( id == 0 ) {
			...
			if ( rodzajFaktury == Faktura.ZAKUP ) {
				if ( !MojeUtils.isNumer( numerField.getText() ) ) {
					throw new Exception( "Podano niepoprawny numer" );
				}
	 ...
	 */
	public void magazynZmienIloscProduktow( int rowIndex, int wartoscZmiany ) throws Exception
	{
		int columnIndex = 2;
		int _ilosc_magazyn =
	 ...
	 * @throws w przypadku odjecia zbyt duzej ilosci rzuca bledem
	 */
	public void aktualizujIloscProduktow_panelMagazyn( ProduktWWysylce produkt, int nowa_wartosc ) throws Exception
	{
