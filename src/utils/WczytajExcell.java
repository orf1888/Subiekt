...
public class WczytajExcell
{
	...
	public static boolean wczytajTowaryZFS() throws Exception
	{
		...
		String zle = new String();//towary z pliku o większej ilości niż w bazie
		String komunikat_koncowy = new String();//niewczytanie_rekordy + zle
		...
		// dodamy do faktury. Te ktorych nie ma w bazie pomijamy i nie zwiekszamy indeksu
		// indeks numerujemy od 1
		int index_produktu = 1;
		for ( int i = 0; i < produkty.length; ++i ) {
			String kodProduktu = produkty[i][0];
			int ilosc_produktu = Integer.parseInt( produkty[i][1] );
			try {
				produktSlownik = (Produkt) ProduktBaza.pobierzObiektZBazyPoKodzie( kodProduktu );
				if ( produktSlownik == null )
					throw new NullPointerException();
			}
			catch ( Exception e ) {
				niewczytane_rekordy +=
					"Produkt o nr " + ( i + 1 ) + " o kodzie \"" + produkty[i][0] + "\"\n";
				continue;
			}
			...
			if ( produktSlownik.ilosc < Integer.parseInt( produkty[i][1] ) ) {
				zle +=
					"Produktu o nr " + ( i + 1 ) + " o kodzie \"" + produkty[i][0]
						+ "\" jest za mało w magazynie!\n" + "magazyn=" + produktSlownik.ilosc
						+ " faktura=" + produkty[i][1] + "\n";
			}
			else {
				listaProduktow.add( new ProduktWFakturze( index_produktu, cena, ilosc_produktu,
		...
		long waluta = 1;
		Faktura nowa = FakturaBaza.nowaFaktura( Faktura.SPRZEDAZ, 0, listaProduktow, waluta );
		FakturaBaza.instance().dodaj( nowa );

		WidokGlowny.frame.panelFakturSprz.tabelaDodajWiersz( nowa.piszWierszTabeli() );
		/*Brak kodu bądź nie rozpoznano produktu*/
		if ( !niewczytane_rekordy.isEmpty() ) {
			komunikat_koncowy +=
				"Produkty znajdujące się na liście nie występują w bazie.\nNależy je dodać ręcznie:\n";
	...
	public static String[][] pobierzProduktyZBazy() throws Exception
	{
		try {
			...
			throw new UserShowException( "Nie udało się pobrać produktów z bazy!" );
		}
	...
	public static String[][] pobierzProduktyZPliku( Sheet arkusz )
	{
		...
		try {
			...
			//for ( int i = 16; i < kody.length - 11; i++ ) {
			int result_index = 0;
			for ( int i = start_end[0]; i <= start_end[1]; i++ ) {
				result[result_index][0] = kody[i].getContents().toString();
				result[result_index][1] = ilosci[i].getContents().toString().replaceAll( ",000", "" );
				++result_index;
			}
		}
		catch ( Exception e ) {
	...
	protected static int[] znajdzPoczatekKoniec( Sheet arkusz, int index_kolumy_lp )
	{
		...
		do {
			try {
				int value = Integer.parseInt( liczbyporzadkowe[index].getContents().toString() );
				if ( value != 0 ) {
					// wczytuje liczbe jesli tam jest, wtedy probuje ustawic pierszą
					// jako START i ostatnią jako END
					if ( start == 0 )
						start = index;
					end = index;
				}
