...
public class Walidator
{
	...
	{
		static final int regon_wagi9[] = { 8, 9, 2, 3, 4, 5, 6, 7 };

		static final int regon_wagi14[] = { 2, 4, 8, 5, 0, 9, 7, 3, 6, 1, 2, 4, 8 };

		static final int nip_wagi[] = { 6, 5, 7, 2, 3, 4, 5, 6, 7 };

		private static boolean isValidRegon9Or14( String regon, int length, int[] wagi )
		{
			int suma = 0;
			int cyfraKontrolna = Character.getNumericValue( regon.charAt( length - 1 ) );
		...
		public static boolean isValidNIP( String nip )
		{
			...
			int suma = 0;
			int cyfraKontrolna = Character.getNumericValue( nip.charAt( size - 1 ) );
	...
	public static int parsujInteger( String text, String nazwaPola ) throws Exception
	{
		...
		try {
			...
			throw new UserShowException( nazwaPola + " - podano niepoprawną liczbę" );
		}
	}

	public static void walidujNrKonta( String numerKonta ) throws UserShowException
	{
		if ( !MojeUtils.isNumer( numerKonta ) )
			throw new UserShowException( "Nuemr konta powinien składać się wyłącznie z cyfr!" );
