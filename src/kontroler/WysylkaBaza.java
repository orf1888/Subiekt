...
public class WysylkaBaza implements ObiektBazaManager
{
	...
	/*
	 * public static int pobierzNrFaktury( int rodzaj ) { try { String querySql
	 ...
	 * new BazaStatementFunktor() {
	 * 
	 * @Override public Object operacja( ResultSet result ) throws SQLException
	 * { try { return result.getInt( 1 ); } catch ( Exception przemilcz ) {
	 * return null; } } } ); } catch ( SQLException e ) { e.printStackTrace();
	 * return -1; } }
	 */
	...
	public void dodaj(Object nowy) throws Exception
	{
		...
		/*
		 * ObiektWyszukanieWarunki warunki = new ObiektWyszukanieWarunki( new
		 * Wysylka() ); warunki.dodajWarunek( nowaWysylka.numer, "numer" );
		...
		/*
		 * } else MojeUtils.showError( "Znaleziono już fakturę o takim numerze"
		 * );
		 */
	...
	public void edytuj(Object stara, Object nowa) throws Exception
	{
		...
		/*
		 * if ( MojeUtils.equals( "" + nowaFaktura.numer, "" +
		 * staraFaktura.numer ) ) { // numer sie nie zmienil - to jedyny
		 * warunek, jest OK } else { // numer sie zmienil - sprawdz czy taki sam
		 * juz nie istnieje w bazie? ObiektWyszukanieWarunki warunki = new
		 * ObiektWyszukanieWarunki( new Faktura() ); warunki.dodajWarunek(
		 * nowaFaktura.numer, "numer" ); warunki.dodajWarunek( 1, "aktualna" );
		 * 
		 * String[][] znalezioneWiersze = pobierzWierszeZBazy( warunki ); if (
		 * znalezioneWiersze.length == 0 ) { // mozna edytować bo nip nie
		 * wystepuje w bazie } else MojeUtils.showError(
		 * "Znaleziono taki sam numer: " + getNumerFromWiersz(
		 * znalezioneWiersze[0] ) ); }
		 */
		...
		BazaDanych.getInstance().aktualizacja(
				"UPDATE "
						+ Wysylka.tableName
						+ " set data= '"
						+ SqlUtils.popraw(nowaWysylka.data_wysylki
								.toGMTString()) + "' WHERE id = "
			...
			throws SQLException
	{
		...
		/*
		 * BazaDanych.getInstance().aktualizacja( "UPDATE " + Wysylka.tableName
		 * + " set aktualna = " + ( aktualna ? 1 : 0 ) + " WHERE id_faktura = "
	...
	/*
	 * public static String getNumerFromWiersz( String[] wiersz ) { return
	 * wiersz[1]; }
	 */
}
