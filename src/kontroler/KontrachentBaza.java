...
public class KontrachentBaza
	implements ObiektBazaManager
{

	private static final String SELECT_QUERY_SQL =
		"SELECT nazwa, miasto, kod_pocztowy, ulica, cena_mnoznik, widoczny, nip, regon, nr_konta, kredyt_kupiecki, id_kontrachent FROM "
	...
	public String[][] pobierzWierszeZBazy( ObiektWyszukanieWarunki warunki ) throws SQLException
	{
		...
		String querySql = "SELECT nazwa,miasto,nip,id_kontrachent FROM " + Kontrachent.tableName;
		...
		// sql do pobrania
			...
			new BazaStatementFunktor() {
				...
				public Object operacja( ResultSet result ) throws SQLException
				{
					...
					// dodajemy do tabeli kolejne pobrane wiersze, skladajace sie z
					// 4 elementow
					while ( result.next() ) {
						String[] wiersz =
	...
	{
		String querySql = SELECT_QUERY_SQL + " WHERE id_kontrachent= " + SqlUtils.popraw( id );
		return pobierzObiektZBazyCore( querySql );
	}

	public static ObiektZId pobierzObiektZBazy( final String nazwa )
	{
		String querySql = SELECT_QUERY_SQL + " WHERE nazwa= '" + SqlUtils.popraw( nazwa ) + "'";
		return pobierzObiektZBazyCore( querySql );
	}
				...
				new BazaStatementFunktor() {
					...
					public Object operacja( ResultSet result ) throws SQLException
					{
						try{
							...
							return new Kontrachent( result.getString( 1 ), result.getString( 2 ),
									result.getString( 3 ), result.getString( 4 ), result.getInt( 5 ),
									result.getBoolean( 6 ), result.getString( 7 ), result.getString( 8 ),
									result.getString( 9 ), result.getInt( 10 ), result.getInt( 11 ) );
						}catch(Exception e)
						{
							return null;
						}
				...
				BazaDanych.getInstance()
							.wstaw(
								"INSERT INTO  "
									...
									+ SqlUtils.popraw( ( nowyKontr.widoczny ? 1
											: 0 ) ) + ",'" + SqlUtils.popraw( nowyKontr.nip ) + "','"
	...
	public void edytuj( Object stary, Object nowy ) throws Exception
	{
		...
		if ( MojeUtils.equals( nowyKontrachent.nip, kontrachent.nip ) ) {
			...
			/*KontrachentWyszukanieWarunki warunki =
			    (KontrachentWyszukanieWarunki) tworzWybrane( null, nowyKontrachent.nip, Globals.Widoczny, Globals.SzukajRownaSie );*/
			String[][] znalezioneWiersze = pobierzWierszeZBazy( warunki );
			if ( znalezioneWiersze.length == 0 ) {
		...
		BazaDanych.getInstance().aktualizacja(
			"UPDATE " + Kontrachent.tableName + " set nazwa= '"
				...
				+ SqlUtils.popraw( nowyKontrachent.miasto ) + "', kod_pocztowy= '"
				...
				+ SqlUtils.popraw( nowyKontrachent.nrKonta ) + "', kredyt_kupiecki = '"
				+ SqlUtils.popraw( nowyKontrachent.kredyt_kupiecki ) + "' WHERE id_kontrachent = "
	...
	{
		BazaDanych.getInstance().aktualizacja(
			"UPDATE " + Kontrachent.tableName + " set widoczny = " + ( widoczny ? 1
					: 0 ) + " WHERE id_kontrachent = " + SqlUtils.popraw( getIdFromWiersz( wiersz ) ) );
	}
	...
	public static String pobierzNazweZBazy( int id ) throws SQLException
	{
		...
		// sql do pobrania
			"SELECT nazwa FROM " + Kontrachent.tableName + " WHERE id_kontrachent= " + id,
			// operacja na pobranych danych
			new BazaStatementFunktor() {
				@Override
				public Object operacja( ResultSet result ) throws SQLException
	...
	public static String[] pobierzWszystkieNazwyZBazy() throws SQLException
	{
		return (String[]) BazaDanych.getInstance().zapytanie(
		// sql do pobrania
			"SELECT nazwa FROM " + Kontrachent.tableName + " WHERE widoczny= 1",
			// operacja na pobranych danych
			new BazaStatementFunktor() {
				@Override
				public Object operacja( ResultSet result ) throws SQLException
	...
	/**
	 * @param dowolnaKolumna - tekst który jest porównywany w każdej kolumnie jaką ma model
	 * @param widoczny - true (tylko widoczne), false (tylko niewidoczne), null (wszystkie)
	 * @param zAsteryskem - czy uwzględniać gwiazdki i zrobić jako LIKE
	 * @return
	 */
	/*public ObiektWyszukanieWarunki tworzPoWszystkim( String dowolnaKolumna, Boolean widoczny, boolean zAsteryskem )
	{
	    KontrachentWyszukanieWarunki result = new KontrachentWyszukanieWarunki( widoczny, zAsteryskem );
	    result.dowolnaKolumna = dowolnaKolumna;
	    return result;
	}

	public ObiektWyszukanieWarunki tworzPoWidocznosci( Boolean widoczny )
	{
	    return new KontrachentWyszukanieWarunki( widoczny, false );
	}

	public ObiektWyszukanieWarunki tworzWybrane( Integer id, String nip, Boolean widoczny, boolean zAsteryskem )
	{
	    KontrachentWyszukanieWarunki result = new KontrachentWyszukanieWarunki( widoczny, zAsteryskem );
	    result.id = id;
	    result.nip = nip;
