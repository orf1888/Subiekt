...
{
	static public ObiektZId pobierzZBazy( final long id, String slownikNazwaTabela )
	{
		try {
			String querySql =
				"SELECT wartosc FROM " + slownikNazwaTabela + " WHERE id= " + SqlUtils.popraw( id );
	...
	static public String[] pobierzWszystkieZBazy( String slownikNazwaTabela, boolean dolaczPustyElement )
	{
		try {
	...
	static public Long pobierzIdZBazy( final String wartosc, String slownikNazwaTabela )
	{
		try {
			String querySql =
				"SELECT id FROM " + slownikNazwaTabela + " WHERE wartosc= '" + SqlUtils.popraw( wartosc )
					+ "'";
	...
	static public String pobierzNazweZBazy( final Long id, String slownikNazwaTabela )
	{
		try {
			String querySql =
				"SELECT wartosc FROM " + slownikNazwaTabela + " WHERE id= '" + SqlUtils.popraw( id + 1 )
					+ "'";
	...
	static public String[] pobierzNazwyZBazy( long id ) throws SQLException
	{
		final List<String> wynik = new ArrayList<String>();
		String querySql = "SELECT wartosc FROM " + Waluta.tabelaWaluta + " WHERE id=" + id;
	...
	/*
			...
			if ( pobierzIdZBazy( nazwaProducenta, SlownikElement.tabelaProducent ) == null ) {
				...
				int generatedId =
					BazaDanych.getInstance().wstaw(
						"INSERT INTO  " + SlownikElement.tabelaProducent + " (wartosc" + ") VALUES ('"
		...
		public static boolean edytuj( String nazwaProducenta, String nowaNazwaProducenta ) throws Exception
		{
			...
			if ( pobierzIdZBazy( nowaNazwaProducenta, SlownikElement.tabelaProducent ) == null ) {
				SlownikElement nowyProducent = new SlownikElement();
				nowyProducent.id = pobierzIdZBazy( nazwaProducenta, SlownikElement.tabelaProducent );
				nowyProducent.wartosc = nowaNazwaProducenta;
				// mozna dodawać bo nazwa unikalna
				BazaDanych.getInstance().aktualizacja(
					"UPDATE " + SlownikElement.tabelaProducent + " set wartosc= '"
						+ SqlUtils.popraw( nowyProducent.wartosc ) + "' WHERE id=" + nowyProducent.id );
				return true;
